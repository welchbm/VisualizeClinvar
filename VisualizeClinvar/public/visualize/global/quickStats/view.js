...
$(function quickStatic(){ //hugely important. defines scope of variables
  ...
  d3.tsv(gon.tsv_path, type, function(error, data) {
    //NOTE gene specific TSV file does not have number of genes, for obvious reasons

    var containerWidth = parseInt(d3.select(".graph").style("width"),0);//select and style give us access to the historyGraphic objects width - kinda of useful for sizing the graphic to fit historyGraphic container on html page
    var containerHeight = parseInt(d3.select(".graph").style("height"),0);
