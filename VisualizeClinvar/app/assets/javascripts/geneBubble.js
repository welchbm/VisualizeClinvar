...
$(document).ready(function () {
  ...
  if($("#gene-bubble-graph").length) {
    ...
    var width = parseInt(d3.select("#gene-bubble-graph").style("width"),0);

    var height = parseInt(d3.select("#gene-bubble-graph").style("width"),0) - 100; // item should be mostly square, only works based off width
    var margin = 50;
    ...
    d3.tsv("../tsv/geneBubble/geneBubble.tsv", function(myData) {
      ...
      var svg=d3.select("#gene-bubble-graph").append("svg")
      ...
      d3.select("#gene-bubble-graph").append("select").attr("id","geneBubblesDates").attr("class", "form-control").append("option").text("Please select a date");
      //d3.select("body")
      //.append("select")
