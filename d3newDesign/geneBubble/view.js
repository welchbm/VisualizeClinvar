...
$(function geneBubbleIt(){ //hugely important. defines scope of variables
  ...
  var width = parseInt(d3.select(".graph.geneBubble").style("width"),0);

  var height = parseInt(d3.select(".graph.geneBubble").style("width"),0) - 100; // item should be mostly square, only works based off width
  var margin = 50;
  ...
  d3.tsv(gon.tsv_paths['geneBubble'],function(myData) {
    ...
    d3.select(".graph.geneBubble").append("select").attr("id","geneBubblesDates").attr("class", "form-control").append("option").text("Please select a date");
    //d3.select("body")
    //.append("select")
